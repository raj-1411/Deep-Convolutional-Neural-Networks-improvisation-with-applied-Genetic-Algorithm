...
def extr(folder_path, out_classes):
    ...
    class refined(nn.Module):
        def __init__(self):
            ...
            initial_model = torchvision.models.vgg19(pretrained=True)
            self.ref_model = nn.Sequential(*list(initial_model.children())[:-1])
            self.rest_linear = nn.Sequential(*list(initial_model.children())[-1:])
            self.flat = nn.Flatten()
    
        def forward(self,images):
            featr = self.flat(self.ref_model(images))
            output = self.rest_linear(featr)
            return featr,output
    ...
    def train(model, criterion, optim, epoch_n):
        ...
        for epoch in range(epoch_n):
            featr_tensor_tr = np.zeros((1,25088))
            labels_ext_tr = []
            model.train()
    ...
    def eval(model, criterion):
        model.eval()
        featr_tensor_val = np.zeros((1,25088))
        labels_ext_val = []
    ...
    model = refined()
    model.rest_linear[0][6] = nn.Linear(model.rest_linear[0][6].in_features, out_classes)
